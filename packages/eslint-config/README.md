...
> 北京先手 JavaScript Typescript Node Vue React 编码规范
> 提供多套配置文件以支持`javascript`、`Typescript`、`Vue`、`React`、`Node.js`等多种项目类型。

## JavaScript 项目 - web-eslit-config
...
> 本配置文件依赖 `@babel/core`、`@babel/eslint-parser`、`eslint-plugin-import` 三个包，因此在使用前需要先安装这三个包。
...
````shell
npm i -D web-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import


### 配置
> 本配置文件的默认导出为 `web-eslint-config`，因此在使用时需要将 `extends` 配置项设置为 `web-eslint-config`。

```json
// eslint.config.mjs
import webEslintConfig from 'web-eslint-config';
...
# JavaScript + React 项目 - web-eslint-config/React
...
````shell
npm i -D web-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import eslint-plugin-react eslint-plugin-react-hooks

### 新版本配置

```json
// eslint.config.mjs
import reactConfig from 'web-eslint-config/react.config.mjs';
...
```json
// eslint.config.mjs
import vueConfig from 'web-eslint-config/vue.config.mjs';
import jsxAllyConfig from 'web-eslint-config/jsx-a11y.config.mjs';
export default [
  ...vueConfig,
  ...jsxAllyConfig,
  // 可以添加其他配置
...
```shell
npm i -D web-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import vue-eslint-parser eslint-plugin-vue
```
...
## JavaScript (Node.js) 项目 - web-eslint-config/node
...
````shell
npm i -D web-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import eslint-config-egg


### 配置

// eslint.config.mjs
import nodeConfig from 'web-eslint-config/node.config.mjs';
...
## TypeScript 项目 - web-eslint-config/typescript
...
````shell
npm i -D web-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript
...
```json
{
  "extends": ["web-eslint-config/typescript"]
}
// eslint.config.mjs
import typescriptConfig from 'web-eslint-config/typescript/index.config.mjs';
export default [
  ...typescriptConfig,
  // 可以添加其他配置
];
...
## TypeScript + React 项目 - web-eslint-config/typescript/React
...
```
npm i -D web-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript eslint-plugin-react eslint-plugin-react-hooks
```
...
```json
// eslint.config.mjs
import reactConfig from 'web-eslint-config/typescript/react.config.mjs';
export default [
  ...reactConfig,
  // 可以添加其他配置
]
...
// eslint.config.mjs
import reactConfig from 'web-eslint-config/typescript/react.config.mjs';
import jsxA11yConfig from 'eslint-config-encode/jsx-a11y.config.mjs';
export default [
  ...reactConfig,
  ...jsxA11yConfig,
  // 可以添加其他配置
...
## TypeScript + Vue 项目 - web-eslint-config/typescript/vue
...
npm i -D web-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript vue-eslint-parser eslint-plugin-vue

### 配置

```json
// eslint.config.mjs
import vueConfig from 'web-eslint-config/typescript/vue.config.mjs';
export default [
  ...vueConfig,
  // 可以添加其他配置
]
...
## TypeScript (Node.js) 项目 - web-eslint-config/typescript/node
...
```
npm i -D web-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript eslint-config-egg
```
...
```json
// eslint.config.mjs
import nodeConfig from 'web-eslint-config/typescript/node.config.mjs';
export default [
  ...nodeConfig,
  // 可以添加其他配置
]
...
```json
{
  "extends": ["web-eslint-config/typescript/react", "prettier"]
}
// eslint.config.mjs
import reactConfig from 'web-eslint-config/typescript/react.config.mjs';
...
为了保证一致的编码风格，本包中大量风格相关的规则被设为了 `error` 级别，以引起开发者的足够重视。如果你觉得风格问题不足以是 `error` 级别（有些用户根据 ESLint error 进行流程卡点），本包还提供了一套名为 'essential' 的配置文件，这套配置将所有风格问题降级为 `warn` 级别，仅将必要问题报告为 `error`，引用方式为在相应配置的 `web-eslint-config` 后面加上 `/essential`，如对 `JS React` 项目为 `web-eslint-config/essential/react`、对 `TS Vue` 项目为 `web-eslint-config/essential/typescript/vue`
...
- 了解如何在继承本包的基础上对项目 ESLint 进行个性化配置，可参考官网的 [Configuring ESLint](https://eslint.org/docs/user-guide/configuring)。下面简介下 ESLint 配置中的几个常用字段：
  - `extends`: 继承一组规则集。`"extends": "web-eslint-config",` 表示继承本包定义的规则配置。
